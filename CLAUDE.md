...
- ✅ **GitHub repo created + pushed** (2026-06-02 pm): `https://github.com/daylayown/matchday-inference` (PRIVATE), `gh` authed as `daylayown`. `git init` done here; `.gitignore` extended to exclude `data/*.sqlite`. Default branch `master`. This is what runs the GH Actions cron.
